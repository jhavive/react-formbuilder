...
export default class RenderForm extends React.Component {
    ...
    render = () => {
        return(
            <Form
                id="form1"
                endpoint="http://localhost:8000/leads/"
                method="POST"
                >
                    <Input type="hidden" name="module" value={this.props.name}/>
                    {
                        this.props.formData && this.props.formData.length > 0 && this.props.formData.map((formItem, index) => (
                            <Input name={spaceToUnderscore(formItem.additional_info[0].value)} placeholder={formItem.additional_info[1].value}/>
                        ))
                    }
